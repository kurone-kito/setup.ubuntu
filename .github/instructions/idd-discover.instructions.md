...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `setup-ubuntu-blocked-by` marker, both resolved the
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- setup-ubuntu-roadmap-id: {roadmap-id} -->`; treat as
...
- **Roadmap identity** (`setup-ubuntu-roadmap-id`): placed in
...
- **Sequential dependency** (`setup-ubuntu-blocked-by`): placed
